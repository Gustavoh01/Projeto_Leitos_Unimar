...
Cliente (Paciente)
Pré-condições
O sistema de automação de leitos está em funcionamento.
O ator relevante (Administrador, Médico, Enfermeiro, Cliente) está logado no sistema com as permissões apropriadas.
Dados básicos (leitos, médicos, enfermeiros) estão cadastrados no sistema, se necessário para a operação específica.
Fluxo Básico
O ator acessa a funcionalidade desejada (Gerenciamento de Leitos, Gerenciamento de Clientes, Gerenciamento de Inventário, etc.).
O sistema exibe a interface correspondente.
O ator realiza a operação desejada (Ex: cadastrar um novo leito, internar um paciente, consultar um inventário).
O sistema processa a requisição e atualiza os dados, se aplicável.
O sistema exibe uma mensagem de sucesso ou o resultado da operação ao ator.
Fluxos Alternativos
Gerenciar Leitos (Administrador)
O Administrador acessa a funcionalidade de "Gerenciamento de Leitos".
Alternativa: Atualizar Leito.
...
O ator acessa a funcionalidade de "Gerenciamento de Clientes".
Alternativa: Adicionar Cliente.
...
O Médico/Enfermeiro acessa a funcionalidade de "Consultar" (pacientes, leitos, etc.).
Alternativa: Filtrar por especialidade/turno.
...
O sistema tenta internar um paciente em um leito que já está ocupado ou bloqueado.
O sistema exibe uma mensagem de erro informando que o leito selecionado está indisponível.
O fluxo retorna para o passo de seleção do leito.
Dados Inválidos/Faltantes no Cadastro
O ator tenta cadastrar um Leito, Cliente, Médico, Enfermeiro ou Item de Inventário com informações inválidas (ex: CPF duplicado, campos obrigatórios em branco) ou em formato incorreto.
O sistema valida os dados e exibe mensagens de erro específicas para cada campo inválido/faltante.
O sistema não permite o cadastro até que os dados sejam corrigidos.
Pós-condições
Os dados do sistema são atualizados de acordo com a operação realizada (leito cadastrado/atualizado/excluído, cliente adicionado/removido, etc.).
O sistema permanece em um estado consistente e pronto para a próxima interação do usuário.
Logs de auditoria são registrados para operações críticas.
...
Referência aos protótipos de média fidelidade desenvolvidos no Figma: https://www.figma.com/design/P2PxSooPLkWKxN4GvndRej/ProjetoLeito?node-id=0-1&amp;t=RpAImhDY96Tt2Jdj-1
